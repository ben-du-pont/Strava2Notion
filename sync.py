...
def sync_activity(activity: Dict, notion_client) -> bool:
    ...
    try:
        ...
        if notion_client.activity_exists(strava_id):
            logger.info(f"Skipping duplicate activity: {activity_name} (ID: {strava_id})")
            return True
        ...
        if sport_type not in ['Run', 'Ride', 'Swim']:
            logger.info(f"Skipping unsupported activity type: {activity_name} (Type: {sport_type})")
            return True
        ...
        if not activity_id:
            logger.error(f"Failed to create activity in Notion: {activity_name}")
            return False
        
        # Find matching planned activity
        activity_date = activity.get('start_date')
        planned_id = notion_client.find_planned_activity(activity_date, sport_type)
        
        if planned_id:
            # Link to planned activity
            link_success = notion_client.link_to_planned_activity(activity_id, planned_id)
            if link_success:
...
def main():
    ...
    try:
        ...
        # Log summary
        logger.info(f"Sync complete: {success_count} succeeded, {failure_count} failed")
