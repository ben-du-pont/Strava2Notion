...
class NotionClient:
    ...
    def activity_exists(self, strava_id: int) -> bool:
        ...
        try:
            ...
            if exists:
                logger.info(
                    f"Activity with Strava ID {strava_id} already exists in Notion")
            return exists
        ...
        try:
            ...
            properties = {
                ...
                "Distance": {
                    ...
                    "number": activity_data.get('distance', 0) / 1000
                },
                "Duration": {
                    # Convert seconds to minutes
                    "number": activity_data.get('moving_time', 0) / 60
                },
                "Elevation": {
            ...
            if sport_type == "Run":
                ...
                avg_speed_kmh = (
                    activity_data.get(
                        'average_speed',
                        0) *
                    3.6) if activity_data.get('average_speed') else 0
                properties["Average Pace"] = {
                    # min/km
                    "number": (60 / avg_speed_kmh) if avg_speed_kmh > 0 else 0
                }
                ...
                if activity_data.get('average_cadence'):
                    properties["Cadence"] = {
                        ...
                        "number": activity_data.get('average_cadence') * 2
                    }

            elif sport_type == "Ride":
                # Cycling-specific fields
                avg_speed_kmh = activity_data.get(
                    'average_speed', 0) * 3.6 if activity_data.get('average_speed') else 0
                ...
                if avg_speed_mps > 0:
                    pace_per_100m = (100 / avg_speed_mps) / \
                        60  # minutes per 100m
                    properties["Average Pace"] = {
    ...
    def find_planned_activity(
        ...
        try:
            ...
            date_only = activity_date.split(
                'T')[0] if 'T' in activity_date else activity_date
            ...
            if results:
                ...
                logger.info(
                    f"Found matching planned activity for {sport_type} on {date_only}")
                return planned_id
            else:
                logger.info(
                    f"No matching planned activity found for {sport_type} on {date_only}")
                return None
    ...
    def link_to_planned_activity(
        ...
        try:
            ...
            logger.info("Successfully linked activity to planned activity")
            return True
    ...
    def update_planned_status(
        ...
        try:
            ...
            logger.info(
                f"Successfully updated planned activity status to {status}")
            return True
